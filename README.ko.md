...
| 유형 | 트리거 | 처리 방식 |
|------|--------|-----------|
| **인프라** | 타임아웃, SIGKILL, SIGSEGV, 바이너리 없음, stderr 비어있음 | 결정론적: PE가 백오프로 재시도 후 레인 실패 처리. |
| **로직** | 의미있는 stderr와 함께 비정상 종료 (테스트 실패, 빌드 에러) | AI 판단: stderr/stdout에서 수리 프롬프트 생성, AI 엔진에 진단 및 수정 요청. `max_replan`으로 제한. |
