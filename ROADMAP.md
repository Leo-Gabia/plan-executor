...
- Goal: stabilize `plan-executor` as an independent multi-agent orchestration runtime. Primary engine is Codex; Gemini and shell serve as secondary/fallback.
- Principles:
...
- Engine support: Codex (`codex exec`, primary), Gemini (`gemini -p --yolo`, secondary), shell (subprocess, fallback).
...
- Done criteria:
  ...
  - Both Codex and Gemini engines complete frontstage consensus without error.
...
- Multi-agent quality:
  ...
  - engine fallback success rate (Codex → Gemini → shell): >= 95%.
